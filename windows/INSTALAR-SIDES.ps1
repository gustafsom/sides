...
function Verify-Package{
  $manifestPath=Join-Path $PackageRoot 'package-manifest.json';$manifestShaPath=Join-Path $PackageRoot 'package-manifest.sha256'
  ...
  foreach($file in $files){
    $path=[string]$file.path;if($seen.ContainsKey($path)){ Fail 'PACKAGE_MANIFEST_DUPLICATE' };$seen[$path]=$true
    $full=Safe-PackagePath $path;if(-not (Test-Path -LiteralPath $full -PathType Leaf)){ Fail "PACKAGE_FILE_MISSING:$path" }
    $info=Get-Item -LiteralPath $full;if([int64]$file.size -ne $info.Length){ Fail "PACKAGE_SIZE_MISMATCH:$path" }
    if(([string]$file.sha256).ToLowerInvariant() -ne (Sha256 $full)){ Fail "PACKAGE_CHECKSUM_MISMATCH:$path" }
  }
  $rootFull=[IO.Path]::GetFullPath($PackageRoot).TrimEnd('\')+'\'
  $actual=@(Get-ChildItem -LiteralPath $PackageRoot -Recurse -File|ForEach-Object{$rel=$_.FullName.Substring($rootFull.Length).Replace('\','/');if($rel -notin @('package-manifest.json','package-manifest.sha256')){$rel}})
  if($actual.Count -ne $files.Count){ Fail 'PACKAGE_FILESET_MISMATCH' };foreach($rel in $actual){if(-not $seen.ContainsKey($rel)){Fail "PACKAGE_UNEXPECTED_FILE:$rel"}}
  return $manifest
}
...
try{
  ...
  $manifest=Verify-Package;$version=[string]$manifest.appVersion;if($version -notmatch '^\d+\.\d+\.\d+'){ Fail 'PACKAGE_VERSION_INVALID' }
  $payload=Join-Path $PackageRoot 'payload';$runtimeNode=Join-Path $payload 'runtime\node.exe'
  if(-not (Test-Path -LiteralPath $runtimeNode)){ Fail 'PORTABLE_NODE_MISSING' };if(-not (Test-Path -LiteralPath (Join-Path $payload 'runtime\NODE-LICENSE.txt'))){ Fail 'PORTABLE_NODE_LICENSE_MISSING' }
  $nodeVersion=(& $runtimeNode -p "process.versions.node").Trim();$parts=$nodeVersion.Split('.')
  ...
  $InstallRoot=[IO.Path]::GetFullPath($InstallRoot);New-Item -ItemType Directory -Path $InstallRoot -Force|Out-Null
  $versionsDir=Join-Path $InstallRoot 'versions';New-Item -ItemType Directory -Path $versionsDir -Force|Out-Null;$statePath=Join-Path $InstallRoot 'install-state.json'
  $oldState=$null;if(Test-Path -LiteralPath $statePath){$oldState=Get-Content -LiteralPath $statePath -Raw|ConvertFrom-Json};if($UpdateOnly -and -not $oldState){ Fail 'SIDES_NOT_INSTALLED' }
  if([string]::IsNullOrWhiteSpace($DataDir)){if($oldState -and $oldState.dataDir){$DataDir=[string]$oldState.dataDir}else{$DataDir=Join-Path $InstallRoot 'data'}}
  ...
  if(Test-Path -LiteralPath $target){if(-not (Test-Path -LiteralPath (Join-Path $target 'package.json'))){ Fail 'EXISTING_VERSION_INVALID' }}else{
    $tempTarget=Join-Path $versionsDir ('.install-'+[Guid]::NewGuid().ToString('N'));New-Item -ItemType Directory -Path $tempTarget -Force|Out-Null
  ...
  $state=[ordered]@{schema='SIDES-INSTALL-V1';version=$version;current=('versions\'+$version);previous=$previous;dataDir=$DataDir;installedAt=$installedAt;updatedAt=(Get-Date).ToUniversalTime().ToString('o');architecture='win-x64'};Write-State $statePath $state
