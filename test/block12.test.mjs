...
function fakePackage(root){
  ...
  for(const path of required)file(root,path,path==='payload/package.json'?JSON.stringify({name:'sides',version:'1.0.0'}):path);
}

test('Windows package manifest is complete, deterministic in file order and verifiable',()=>{
  const dir=temp();try{
    fakePackage(dir);
    const manifest=writeWindowsManifest(dir,{version:'1.0.0',createdAt:new Date('2026-08-28T23:00:00Z')});
    assert.equal(manifest.schema,WINDOWS_PACKAGE_SCHEMA);assert.equal(manifest.appVersion,'1.0.0');
    assert.deepEqual(manifest.files.map(x=>x.path),[...manifest.files.map(x=>x.path)].sort());
    const verified=verifyWindowsPackage(dir);assert.equal(verified.ok,true);assert.equal(verified.version,'1.0.0');
  }finally{rmSync(dir,{recursive:true,force:true});}
});

test('package verification detects modified or unexpected files',()=>{
  const dir=temp();try{
    fakePackage(dir);writeWindowsManifest(dir,{version:'1.0.0'});
...
test('manifest rejects traversal, duplicate paths and missing portable layout',()=>{
  ...
  const base={schema:WINDOWS_PACKAGE_SCHEMA,app:'SIDES',appVersion:'1.0.0',architecture:'win-x64',fileCount:1,files:[{path:'../evil',size:1,sha256:'a'.repeat(64)}]};
  assert.equal(validateWindowsManifest(base).error,'PACKAGE_PATH_INVALID');
  const dir=temp();try{
    file(dir,'payload/package.json','{}');
    const m=buildWindowsManifest(dir,{version:'1.0.0'});
...
test('Windows launcher separates immutable versions from persistent data and has no GitHub runtime dependency',()=>{
  ...
  const rollback=readFileSync(new URL('../windows/launcher/Rollback-SIDES.ps1',import.meta.url),'utf8');
  ...
  assert.match(install,/CONFIGURAR-VOZ-OFFLINE\.ps1/);assert.match(install,/CONFIGURAR-GRAMATICA-LOCAL\.ps1/);assert.match(install,/Rollback-SIDES\.ps1/);
  ...
  assert.match(rollback,/previous/);assert.match(rollback,/dataDir/);assert.equal(/Remove-Item[^\n]+dataDir/i.test(rollback),false);
...
test('Windows build recipe embeds Node licenses and dependency but never packages data directory',()=>{
  const build=readFileSync(new URL('../BUILD-WINDOWS-PACKAGE.ps1',import.meta.url),'utf8');
  assert.match(build,/runtime\\node\.exe/);assert.match(build,/NODE-LICENSE\.txt/);assert.match(build,/THIRD_PARTY_NOTICES\.md/);
  assert.match(build,/node_modules\\ts-fsrs/);assert.match(build,/Compress-Archive/);
