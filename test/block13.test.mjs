...
test('CURESP 1.0.0 preserves DB/API/export compatibility instead of introducing a destructive migration',()=>{
  const pkg=JSON.parse(read('package.json'));assert.equal(pkg.name,'curesp');assert.equal(pkg.version,'1.0.0');assert.equal(pkg.dependencies['ts-fsrs'],'5.4.1');
...
test('security audit executable passes against the CURESP release source tree',()=>{
  const r=spawnSync(process.execPath,['scripts/security-audit.mjs'],{encoding:'utf8'});
  assert.equal(r.status,0,r.stdout+'\n'+r.stderr);assert.match(r.stdout,/CURESP_SECURITY_AUDIT_OK/);assert.match(r.stdout,/"high": 0/);
});

test('release static gate executable passes against the exact CURESP 1.0 source contract',()=>{
  const r=spawnSync(process.execPath,['scripts/release-gate.mjs'],{encoding:'utf8'});
  assert.equal(r.status,0,r.stdout+'\n'+r.stderr);assert.match(r.stdout,/CURESP_RELEASE_GATE_OK/);assert.match(r.stdout,/"version": "1\.0\.0"/);
});
