...
test('SIDES 1.0.0 preserves DB/API/export compatibility instead of introducing a destructive migration',()=>{
  const pkg=JSON.parse(read('package.json'));assert.equal(pkg.version,'1.0.0');assert.equal(pkg.dependencies['ts-fsrs'],'5.4.1');
...
test('release documentation and notices cover security, acceptance, licenses and changelog',()=>{
  assert.match(read('CHANGELOG.md'),/## \[1\.0\.0\] - 2026-08-28/);
  ...
  assert.match(read('docs/ACCEPTANCE-1.0.md'),/SISDEV-GATE\.ps1/);
...
test('rollback is fail-closed, switches only version pointers and keeps persistent data directory',()=>{
  ...
  assert.match(rollback,/current=\$previous/);assert.match(rollback,/previous=\$current/);assert.match(rollback,/dataDir=\[string\]\$state\.dataDir/);
  assert.equal(/Remove-Item[^\n]*(?:dataDir|sides\.sqlite)/i.test(rollback),false);
});

test('SISDEV gate is exact-SHA, clean-tree, READ_ONLY and publishes sanitized result contract',()=>{
  const gate=read('SISDEV-GATE.ps1');
  assert.match(gate,/ValidatePattern\('\^\[0-9a-fA-F\]\{40\}\$'\)/);assert.match(gate,/rev-parse HEAD/);assert.match(gate,/status --porcelain/);
  assert.match(gate,/mode='READ_ONLY'/);assert.match(gate,/pipeline='sides-release-local'/);assert.match(gate,/SISDEV-SIDES-GATE-V1/);
  assert.match(gate,/Downloads\\SISDEV\\RESULTADOS\\sides/);
...
test('security audit executable passes against the release source tree',()=>{
  const r=spawnSync(process.execPath,['scripts/security-audit.mjs'],{encoding:'utf8'});
  assert.equal(r.status,0,r.stdout+'\n'+r.stderr);assert.match(r.stdout,/SIDES_SECURITY_AUDIT_OK/);assert.match(r.stdout,/"high": 0/);
});

test('release static gate executable passes against the exact 1.0 source contract',()=>{
  const r=spawnSync(process.execPath,['scripts/release-gate.mjs'],{encoding:'utf8'});
  assert.equal(r.status,0,r.stdout+'\n'+r.stderr);assert.match(r.stdout,/SIDES_RELEASE_GATE_OK/);assert.match(r.stdout,/"version": "1\.0\.0"/);
});
