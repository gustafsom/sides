...
test('CURESP 1.x preserves DB/API/export compatibility instead of introducing a destructive migration',()=>{
  const current=pkg();assert.equal(current.name,'curesp');assert.match(current.version,/^1\.\d+\.\d+$/);assert.equal(current.dependencies['ts-fsrs'],'5.4.1');
...
test('release documentation and notices cover CURESP identity, security, acceptance, licenses and changelog',()=>{
  const version=regexEscape(pkg().version);
  assert.match(read('CHANGELOG.md'),new RegExp(`## \\[${version}\\] - Em desenvolvimento`));
...
test('security audit executable passes against the current CURESP 1.x source tree',()=>{
  const r=spawnSync(process.execPath,['scripts/security-audit.mjs'],{encoding:'utf8'}),version=regexEscape(pkg().version);
  assert.equal(r.status,0,r.stdout+'\n'+r.stderr);assert.match(r.stdout,/CURESP_SECURITY_AUDIT_OK/);assert.match(r.stdout,/"high": 0/);assert.match(r.stdout,new RegExp(`"version": "${version}"`));
});

test('release static gate executable passes against the current CURESP 1.x source contract',()=>{
  const r=spawnSync(process.execPath,['scripts/release-gate.mjs'],{encoding:'utf8'}),version=regexEscape(pkg().version);
  assert.equal(r.status,0,r.stdout+'\n'+r.stderr);assert.match(r.stdout,/CURESP_RELEASE_GATE_OK/);assert.match(r.stdout,new RegExp(`"version": "${version}"`));
});
