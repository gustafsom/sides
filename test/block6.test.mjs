...
test('V6 preserves versioned assignment tables without audio persistence columns',()=>{
...
test('assignment planner creates spaced phases through the real due date',()=>{
  ...
  assert.equal(item.readiness,0);
  assert.equal(item.plan.phase,'foundation');
  assert.equal(item.plan.daysRemaining,23);
  assert.ok(item.plan.schedule.length>=10);
  assert.equal(item.plan.schedule[0].date,'2026-08-28');
  assert.equal(item.plan.schedule.at(-1).date,'2026-09-20');
  assert.ok(item.plan.schedule.some(x=>x.phase==='build'));
  assert.ok(item.plan.schedule.some(x=>x.phase==='rehearsal'));
  assert.ok(item.plan.schedule.some(x=>x.phase==='final'));
});
...
test('rehearsal raises readiness and weakest rubric becomes explicit focus',()=>{
  ...
  assert.ok(after.readiness>0);
  assert.equal(after.practiceCount,1);
  assert.equal(after.status,'practicing');
  assert.equal(after.plan.focusRubrics[0].key,'naturalidad');
  assert.equal(after.plan.focusRubrics[0].average,2);
  assert.ok(after.xp>=8);
  const mastery=db.prepare("SELECT * FROM skill_mastery WHERE skill_type='jw' AND skill_key='rubric:naturalidad'").get();
  assert.ok(mastery);
});

test('multiple good rehearsals can mark an assignment ready and history remains intact',()=>{
  const db=openDatabase(':memory:');
  const item=createAssignment(db,{type:'comment',title:'Comentário',dueDate:'2026-09-02',targetSeconds:45},NOW);
  let current=item;
  for(let i=0;i<5;i++){
    const when=new Date(NOW.getTime()+i*86400000);
    current=recordAssignmentPractice(db,item.id,{durationMs:44000,confidence:5,ratings:{naturalidad:5,ritmo:4,sencillez:5,conviccion:4}},when);
  }
  assert.ok(current.readiness>=75);
  assert.equal(current.status,'ready');
  assert.equal(current.practiceCount,5);
  const completed=updateAssignment(db,item.id,{status:'completed'},new Date('2026-09-02T12:00:00Z'));
  assert.equal(completed.status,'completed');
  assert.equal(completed.practiceCount,5);
});

test('cancel is non-destructive and removes item from active/list view only',()=>{
  const db=openDatabase(':memory:');
  const item=createAssignment(db,{type:'student',title:'Designação',dueDate:'2026-09-10'},NOW);
  recordAssignmentPractice(db,item.id,{durationMs:120000,confidence:3,ratings:{naturalidad:3}},NOW);
  updateAssignment(db,item.id,{status:'cancelled'},NOW);
  assert.equal(listAssignments(db,NOW).some(x=>x.id===item.id),false);
  const preserved=getAssignment(db,item.id,NOW);
  assert.equal(preserved.status,'cancelled');
  assert.equal(preserved.practiceCount,1);
});
...
test('assignment input validation rejects malformed dates, times and types',()=>{
  ...
  assert.throws(()=>createAssignment(db,{title:'X',dueDate:'2026-02-31'},NOW),/ASSIGNMENT_DATE_INVALID/);
  assert.throws(()=>createAssignment(db,{title:'X',dueDate:'2026-09-01',dueTime:'25:10'},NOW),/ASSIGNMENT_TIME_INVALID/);
  assert.throws(()=>createAssignment(db,{title:'',dueDate:'2026-09-01'},NOW),/ASSIGNMENT_TITLE_REQUIRED/);
});

test('overview prioritizes nearest future assignment and reports readiness totals',()=>{
  const db=openDatabase(':memory:');
  createAssignment(db,{type:'talk',title:'Mais tarde',dueDate:'2026-09-20'},NOW);
  const soon=createAssignment(db,{type:'reading',title:'Primeiro',dueDate:'2026-08-30'},NOW);
  const overview=assignmentOverview(db,NOW);
  assert.equal(overview.active,2);
  assert.equal(overview.next.id,soon.id);
...
test('HTTP assignment API creates data and backup includes assignment history',async()=>{
  ...
  try{
    ...
    assert.equal(health.schema,'SIDES-API-V5');
    ...
    assert.equal(backup.schemaVersion,'SIDES-EXPORT-V5');
