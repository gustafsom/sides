...
function record(db,payload,metrics,now){
  ...
  const result=db.prepare(`INSERT INTO speech_attempts(context_type,context_id,duration_ms,expected_words,recognized_words,correct_words,
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(contextType,contextId,Math.max(0,Number(payload.durationMs||payload.audioStats?.durationMs||0)),metrics.expectedWords,
      metrics.recognizedWords,metrics.correctWords,metrics.counts.omit,metrics.counts.add,metrics.counts.substitute,metrics.counts.accent,metrics.accuracy,
      metrics.strictAccuracy,metrics.wpm,metrics.pauseCount,metrics.longPauses,metrics.maxPauseMs,metrics.totalSilenceMs,now.toISOString());
  ...
  db.prepare('UPDATE activity SET xp=xp+?,attempts=attempts+1,correct=correct+?,speaking=speaking+1,minutes=minutes+? WHERE day=?')
    .run(xp,metrics.accuracy>=70?1:0,Math.max(0,Math.round(Number(payload.durationMs||0)/60000)),day);
...
export function speechOverview(db,days=30,now=new Date()){
  ...
  return {days:safeDays,count:rows.length,averageAccuracy:avg('accuracy'),averageWpm:avg('wpm'),averageLongPauses:avg('long_pauses'),skills,
    recent:rows.slice(0,12).map(x=>({id:x.id,contextType:x.context_type,durationMs:x.duration_ms,accuracy:x.accuracy,wpm:x.wpm,omissions:x.omissions,substitutions:x.substitutions,longPauses:x.long_pauses,createdAt:x.created_at})),
    runtime:speechRuntimeStatus()};
}
