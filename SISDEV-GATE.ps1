...
Set-Location $Root
$ResultRoot=Join-Path $env:USERPROFILE 'Downloads\SISDEV\RESULTADOS\sides'
$ResultFile=Join-Path $ResultRoot 'SIDES-GATE-safe.json'
$BuildRoot=Join-Path ([IO.Path]::GetTempPath()) ('sides-sisdev-'+[guid]::NewGuid().ToString('N'))
...
function Write-SafeResult([bool]$Ok,[string]$Code){
  ...
  [ordered]@{
    schema='SISDEV-SIDES-GATE-V1';system='sides';pipeline='sides-release-local';mode='READ_ONLY';exactSha=$ExactSha.ToLowerInvariant();
...
try{
  ...
  Write-Host "SISDEV gate aprovado para o SHA exato $ExactSha" -ForegroundColor Green
  Write-Host "Resultado sanitizado: %USERPROFILE%\Downloads\SISDEV\RESULTADOS\sides\SIDES-GATE-safe.json" -ForegroundColor Cyan
  exit 0
}catch{
  Write-SafeResult $false $_.Exception.Message
  Write-Error ("SISDEV gate falhou: "+$_.Exception.Message)
  exit 1
